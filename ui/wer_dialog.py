"""WER/CER measurement dialog — compare hypothesis vs reference."""
...
class WERDialog(QDialog):
    def __init__(self, parent=None) -> None:
        ...
        self.setWindowTitle("WER / CER 측정")
        self.resize(700, 500)
        self._build_ui()
    ...
    def _build_ui(self) -> None:
        ...
        btn_calc = QPushButton("▶ 측정")
        btn_calc.clicked.connect(self._calculate)
        root.addWidget(btn_calc)

        result_grp = QGroupBox("결과")
        res_layout = QHBoxLayout(result_grp)
        ...
        res_layout.addWidget(self._wer_label)
        res_layout.addWidget(self._cer_label)
    ...
    def _load_text(self, editor: QTextEdit) -> None:
        path, _ = QFileDialog.getOpenFileName(self, "파일 선택", "", "Text (*.txt *.srt)")
        if path:
            editor.setPlainText(Path(path).read_text(encoding="utf-8"))

    def _load_hyp(self) -> None:
        path, _ = QFileDialog.getOpenFileName(self, "전사 결과 파일 선택", "", "Text/SRT (*.txt *.srt)")
        if not path:
            return
    ...
    def _strip_srt(self, srt_text: str) -> str:
        """Extract plain text lines from SRT."""
        lines = []
    ...
    def _calculate(self) -> None:
        ...
        try:
            ...
            self._wer_label.setText("jiwer 패키지 미설치")
