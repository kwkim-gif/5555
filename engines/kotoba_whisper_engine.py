"""Kotoba-Whisper-v2 engine using Faster-Whisper backend (HF pipeline fallback)."""
...
class KotobaWhisperEngine(BaseSTTEngine):
    ...
    Prefers Faster-Whisper (CTranslate2) backend; falls back to HF pipeline.
    """
    ...
    def load_model(self) -> None:
        logger.info(f"[KotobaWhisper] Loading on {self.device} ({self.compute_dtype})")
        try:
            self._load_faster_whisper()
            self._use_faster = True
            logger.info("[KotobaWhisper] Faster-Whisper backend loaded")
        except Exception as exc:
            logger.warning(f"[KotobaWhisper] Faster-Whisper unavailable ({exc}), falling back to HF")
    ...
    def transcribe(
        ...
        logger.info(f"[KotobaWhisper] Transcribing: {Path(audio_path).name}")
    ...
    def unload_model(self) -> None:
        ...
        logger.info("[KotobaWhisper] Unloaded")
    ...
    def _transcribe_faster(
        self, audio_path: str, progress_callback: Optional[callable]
    ) -> Generator[TranscriptionSegment, None, TranscriptionResult]:
        ...
        for seg in segments_iter:
            ...
            ] if self.config.word_timestamps else []
            segment = TranscriptionSegment(
                start=seg.start, end=seg.end, text=seg.text.strip(),
                confidence=seg.avg_logprob, words=words,
            )
        ...
        return TranscriptionResult(segments=collected, language=info.language, duration=total_duration)
    ...
    def _load_hf_pipeline(self) -> None:
        ...
        model = AutoModelForSpeechSeq2Seq.from_pretrained(
            ...
            low_cpu_mem_usage=True, use_safetensors=True, cache_dir=".model_cache",
        )
    ...
    def _transcribe_hf(
        ...
        return TranscriptionResult(segments=collected, language="ja", duration=total_duration)
